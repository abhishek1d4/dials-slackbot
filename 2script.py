...
def main():
    user_input = input("Enter the ticket details:").strip()
    
    accepted_inputs = {
    "DIAL_ID": r"^DIAL-\d+$",  # e.g., DIAL-16565
    ...
    # Match input against the patterns
    for input_type, pattern in accepted_inputs.items():
        match = re.match(pattern, user_input)
        if match:
            matched_type = input_type
            extracted_value = user_input
            break

    if not matched_type:
        print("Invalid input: Input does not match any accepted pattern.")
        return

    # Process the matched input type
    if matched_type == "DIAL_ID":
        # dial_handle()
        print(f"Processing DIAL ID: {extracted_value}")
        dial_handle(extracted_value)

    elif matched_type == "JITA_URL":
        # print(f"Processing as {matched_type}: Provisional ID: {extracted_value}")
        # Fetch log link for the extracted Provisional ID
        # rdm_log_link = get_log_link(extracted_value, rdm_api_base_url="https://rdm.eng.nutanix.com/")
        # return f"Processed {matched_type}: {user_input}, Log Link: {rdm_log_link}"

        flag=0
        jita_link=extracted_value
        print(f"JITA Link found: ",jita_link)
        flag=check_deployment_failure(jita_link)
        if flag==0:
            print(f"------------->>>>>>>>> No DEPLOYMENT FAILURE DETECTED <<<<<<<<<<-------------")
    ...
    elif matched_type == "RDM_URL":
        provisional_id=extract_provisional_id(extracted_value)
        if provisional_id :
    ...
    elif matched_type == "PROVISIONAL_ID":
        print(f"Processing as Provisional ID: {extracted_value}")
        find_deployment_links(extracted_value)
